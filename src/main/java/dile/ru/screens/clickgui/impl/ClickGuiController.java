package dile.ru.screens.clickgui.impl;
...
import dile.ru.screens.clickgui.impl.panel.ClickGuiPanel;
import dile.ru.screens.clickgui.impl.panel.ClickGuiPanelSlot;
...
public final class ClickGuiController {
    ...
    private static final List<ClickGuiPanel> PANELS = new ArrayList<>(List.of(
            ...
            new ClickGuiPanel(ModuleCategory.AUTO_BUY)
    ));
    ...
    private static float cachedBaseY;
    ...
    public static void renderPanels(GuiGraphics graphics, int screenWidth, int screenHeight) {
        ...
        SEARCH_BAR.render(graphics, designScreenWidth, openProgress, 0.0f, baseY + scaledPanelHeight + 7.0f);
        Render2D.flush();
    }
    ...
    public static boolean keyPressed(KeyEvent event) {
        if (SEARCH_BAR.keyPressed(event)) {
            return true;
        }
    ...
    public static boolean mouseClicked(MouseButtonEvent event, boolean doubled) {
        ...
        mouseX = event.x();
        mouseY = event.y();
        if (!closing && SEARCH_BAR.mouseClicked(event, doubled)) {
            return true;
        }
