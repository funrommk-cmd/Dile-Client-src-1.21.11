...
public class ModuleOption {
    ...
    private final SmoothAnimation searchAnimation = new SmoothAnimation();
    ...
    private float searchProgress = 1.0f;
    private float rowX;
    private float rowY;
    ...
    public void updateSearchMatch() {
        String query = ClickGuiSearchBar.getQuery();
        if (query == null || query.isBlank()) {
            matchesSearch = true;
        } else {
            String normalized = ClickGuiSearchBar.normalize(query);
            String moduleName = ClickGuiSearchBar.normalize(module.getName());
            matchesSearch = moduleName.contains(normalized);
            if (!matchesSearch) {
                for (var setting : module.getSettings()) {
                    if (ClickGuiSearchBar.normalize(setting.getName()).contains(normalized)) {
                        matchesSearch = true;
                        break;
                    }
                }
    ...
    public void updateAnimation() {
        ...
        searchAnimation.run(matchesSearch ? 1.0 : 0.0, 0.18, Easings.CUBIC_OUT, true);
        searchAnimation.update();
        searchProgress = (float) searchAnimation.get();
    ...
    public void renderRow(float panelX, float rowY, float panelWidth, float rowHeight, float scale, float alpha) {
        ...
        this.rowInteractive = sa > 0.01f && panelWidth > 0.0f && this.rowHeight > 0.0f;

        float enabledProgress = enabledAnimation.get();
        int moduleColor = Math.round(185.0f + 70.0f * enabledProgress);
        int indicatorColor = ColorUtil.withAlpha(getThemeColor(), (int) (160 * enabledProgress * (1.0f - bindProgress) * sa));
        int textColor = color(moduleColor, moduleColor, moduleColor, (int) (255 * (1.0f - bindProgress)), sa);
        float moduleTextX = panelX + 11.0f * scale;
        float moduleTextY = rowY + 5.3f * scale;
        float moduleTextWidth = Render2D.textWidth(FontType.SEMIBOLD, module.getName(), 9.0f);
        this.rowClickX = panelX;
        this.rowClickY = rowY;
        this.rowClickWidth = panelWidth;
        this.rowClickHeight = rowHeight * scale;

        float bgX = panelX + 3.0f * scale;
        float bgY = rowY + 1.5f * scale;
        float bgW = panelWidth - 6.0f * scale;
        float bgH = rowHeight * scale - 3.0f * scale;
        float bgRadius = 4.0f * scale;
        Render2D.rect(bgX, bgY, bgW, bgH, bgRadius, color(0, 0, 0, (int) (50 * sa), sa));
        int outlineColor = ColorUtil.withAlpha(getThemeColor(), (int) (40 * enabledProgress * sa));
        Render2D.outline(bgX, bgY, bgW, bgH, bgRadius, 1.0f * scale, outlineColor);

        Render2D.rect(panelX + 7.5f * scale, rowY + 6.5f * scale, 1.0f * scale, 9.0f * scale, 0, indicatorColor);
        Render2D.text(FontType.SEMIBOLD, module.getName(), moduleTextX, moduleTextY, 9.0f, textColor);

        if (hasSettings()) {
            float settingsIconX = moduleTextX + moduleTextWidth + 3.4f;
            float settingsIconY = moduleTextY;
            Render2D.text(FontType.HUD2, "i", settingsIconX, settingsIconY, 9.0f, textColor);
        }
    ...
    public float getAnimatedExtraHeight() {
        if (!hasSettings() || searchProgress <= 0.01f) {
            return 0.0f;
        }
        float settingsHeight = computeSettingsHeight();
        if (settingsHeight <= 0.01f) {
        ...
        return (settingsHeight + AFTER_OPTIONS_GAP + 2.0f) * settingsProgress * searchProgress;
    }
