...
public final class SettingCardController {
    private static final int MODULE_LIST_ANIMATION_MS = 300;
    ...
    public void render(GuiGraphics graphics, float x, float y, float panelWidth, float panelHeight, ModuleCategory category, boolean searchMode, List<Module> modules, int mouseX, int mouseY) {
        ...
        if (isAutoBuyScope(category, searchMode)) {
            renderAutoBuyPanel(graphics, x, y, panelWidth, panelHeight, displayedModules, categoryProgress, mouseX, mouseY);
            return;
        }
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean doubled) {
        ...
        if (autoBuyModule != null && autoBuyPicker.mouseClicked(event, context, autoBuyModule)) {
            return true;
        }
        ...
        if (handleModuleRightClick(event)) {
            return true;
        }
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double scrollX, double scrollY) {
        ...
        moduleScrollTarget = Math.max(0.0F, moduleScrollTarget - (float) scrollY * 20.0F);
        saveScrollScope();
        return true;
    }
    ...
    public void resetScroll() {
        ...
        CATEGORY_SCROLLS.clear();
        SEARCH_SCROLL.current = 0.0F;
        SEARCH_SCROLL.target = 0.0F;
        moduleListInitialized = false;
        activeCategory = null;
        activeSearchMode = false;
        pendingCategory = null;
        pendingSearchMode = false;
        displayedModules = List.of();
    ...
    private void renderModuleCards(GuiGraphics graphics, float x, float y, float panelWidth, float panelHeight, List<Module> modules, float categoryProgress, int mouseX, int mouseY) {
        ...
        for (ModuleLayoutEntry entry : layoutEntries) {
            ...
            state.currentSettings = entry.settings;
            float entryHeight = state.animatedCardHeight(context);
            if (leftH <= rightH) {
                leftH += entryHeight + cardGap;
            } else {
                rightH += entryHeight + cardGap;
            }
        }
        moduleContentHeight = Math.max(leftH, rightH) - cardGap + 17.7F;
        moduleScrollTarget = Math.max(0.0F, moduleScrollTarget);
        ...
        for (ModuleLayoutEntry entry : layoutEntries) {
            ...
            ClickGuiModule animMod = ClickGuiModule.getInstance();
            boolean animateModules = animMod != null && animMod.isModuleAnimationEnabled();
            if (!state.includedLastFrame) {
                if (animateModules) {
                    int entryIndex = layoutEntries.indexOf(entry);
                    long staggerMs = entryIndex * 15L;
                    state.entryAnimation.counter.setTime(System.currentTimeMillis() + staggerMs);
                } else if (firstListRender) {
            ...
            state.cardHeight = state.animatedCardHeight(context);
            ...
            if (leftColumn) {
                leftOffset += state.cardHeight + cardGap;
                leftLayoutOffset += entry.layoutHeight + cardGap;
            } else {
                rightOffset += state.cardHeight + cardGap;
    ...
    private boolean isAutoBuyScope(ModuleCategory category, boolean searchMode) {
        return !searchMode && category == ModuleCategory.AUTO_BUY
                && activeCategory == ModuleCategory.AUTO_BUY
                && !displayedModules.isEmpty();
    }

    private void renderAutoBuyPanel(GuiGraphics graphics, float x, float y, float panelWidth, float panelHeight, List<Module> modules, float categoryProgress, int mouseX, int mouseY) {
        if (modulesChanged(modules)) {
            lastRenderModules = new ArrayList<>(modules);
        }
        for (ModuleState state : moduleStates.values()) {
        ...
        AHHelper module = null;
        for (Module candidate : modules) {
            if (candidate instanceof AHHelper autoBuy) {
                module = autoBuy;
                break;
            }
        }
        autoBuyModule = module;

        List<SettingCardComponent<?>> settings = new ArrayList<>();
        for (Module candidate : modules) {
            for (Setting<?> setting : candidate.getSettings()) {
                SettingCardComponent<?> component = componentFor(setting);
                if (component != null) {
                    component.updateVisibility();
                }
            }
            settings.addAll(visibleComponents(candidate));
            ModuleState state = moduleState(candidate);
            state.includedThisFrame = true;
            state.visible = false;
            state.column = -1;
        }
        ...
        float rightX = x + 215.0F;
        float startY = y + 48.0F + (1.0F - categoryProgress) * 15.0F - moduleScroll;
        float pickerHeight = module == null ? 0.0F : autoBuyPicker.measure(panelWidth - 60.0F);
        float settingsStartY = startY + pickerHeight;

        float leftLayout = 0.0F;
        float rightLayout = 0.0F;
        float leftColumnH = 0.0F;
        float rightColumnH = 0.0F;
        for (SettingCardComponent<?> component : settings) {
            float layoutHeight = component.animatedLayoutHeight(context);
            if (leftLayout <= rightLayout) {
                leftLayout += layoutHeight + cardGap;
                leftColumnH += component.animatedHeight(context) + cardGap;
            } else {
                rightLayout += layoutHeight + cardGap;
                rightColumnH += component.animatedHeight(context) + cardGap;
            }
        }
        moduleContentHeight = pickerHeight + Math.max(leftColumnH, rightColumnH) - cardGap + 17.7F;
        moduleScrollTarget = Math.max(0.0F, moduleScrollTarget);
        moduleScroll += (moduleScrollTarget - moduleScroll) * 0.22F;
        saveScrollScope();
        ...
        context.setVisualAlpha(previousAlpha * categoryProgress);

        if (module != null) {
            autoBuyPicker.render(graphics, context, context.moduleClipX, startY, context.moduleClipWidth, mouseX, mouseY, module);
        }
        ...
        for (SettingCardComponent<?> component : settings) {
            ...
            boolean leftColumn = leftLayout <= rightLayout;
            float cardX = leftColumn ? leftX : rightX;
            float settingY = settingsStartY + (leftColumn ? leftOffset : rightOffset);
            if (settingY + height >= context.moduleClipY - 5.0F && settingY <= context.moduleClipY + context.moduleClipHeight + 5.0F) {
                renderInnerCard(cardX, settingY, 150.0F, height);
                component.renderAnimated(graphics, context, cardX, settingY, mouseX, mouseY);
            }
            if (leftColumn) {
                leftOffset += height + cardGap;
                leftLayout += component.animatedLayoutHeight(context) + cardGap;
            } else {
                rightOffset += height + cardGap;
                rightLayout += component.animatedLayoutHeight(context) + cardGap;
            }
        }
        for (SettingCardComponent<?> component : settings) {
    ...
    private void renderModuleCard(GuiGraphics graphics, Module module, List<SettingCardComponent<?>> settings, ModuleState state, float alpha, int mouseX, int mouseY) {
        ...
        state.settingsExpandAnimation.setDirection(state.settingsExpanded ? Direction.FORWARDS : Direction.BACKWARDS);
        float expandProgress = context.clamp(state.settingsExpandAnimation.getOutput().floatValue(), 0.0F, 1.0F);
        float previousAlpha = context.visualAlpha();
        context.setVisualAlpha(previousAlpha * alpha);

        ClickGuiModule animMod = ClickGuiModule.getInstance();
        boolean animateModules = animMod != null && animMod.isModuleAnimationEnabled();
        float entryProgress = context.clamp(state.entryAnimation.getOutput().floatValue(), 0.0F, 1.0F);
        if (animateModules && entryProgress < 1.0F) {
            int strips = 6;
            float stripH = state.cardHeight / strips;
            for (int i = 0; i < strips; i++) {
                float stripP = context.clamp((entryProgress * strips - i) / 1.3f, 0.0F, 1.0F);
                if (stripP <= 0.001F) continue;

                float sy = cardY + i * stripH;
                Render2D.pushScissor(graphics, cardX, sy, state.cardWidth, stripH);

                float shake = (1.0F - stripP) * (1.0F - stripP) * 5.0F;
                if ((i & 1) == 0) shake = -shake;

                graphics.pose().pushMatrix();
                graphics.pose().translate(shake, 0);
        ...
        if (expandProgress > 0.01F) {
            Render2D.pushScissor(graphics, cardX, cardY + 22.0F, state.cardWidth, (state.cardHeight - 22.0F) * expandProgress);
            float settingY = cardY + 24.0F;
            for (SettingCardComponent<?> component : settings) {
                float height = component.animatedHeight(context) * expandProgress;
                if (settingY + height >= context.moduleClipY - 5.0F && settingY <= context.moduleClipY + context.moduleClipHeight + 5.0F) {
                    component.renderAnimated(graphics, context, cardX, settingY, mouseX, mouseY);
                }
                settingY += height;
    ...
    private void renderModuleHeader(GuiGraphics graphics, Module module, ModuleState state) {
        ...
        float moduleNameWidth = Render2D.textWidth(FontType.SEMIBOLD, module.getName(), moduleNameSize);
        Render2D.text(
                FontType.SEMIBOLD,
                module.getName(),
        ...
        if (!module.getSettings().isEmpty()) {
            Render2D.text(
                    FontType.HUD2,
                    "i",
                    moduleNameX + moduleNameWidth + 3.4F, moduleNameY,
                    moduleNameSize,
                    context.color(255, 255, 255, 255)
            );
        }
        ...
        if (animation > 0.01F) {
            Render2D.rect(
                    ...
                    state.toggleHeight,
                    4,
                    context.withAlpha(SettingCardContext.themeColor(), animation),
                    context.withAlpha(SettingCardContext.themeLighter(), animation),
                    context.withAlpha(SettingCardContext.themeColor(), animation),
                    context.withAlpha(SettingCardContext.themeDarker(), animation)
            );
        } else {
            Render2D.rect(
                    state.toggleX,
    ...
    private boolean handleModuleRightClick(MouseButtonEvent event) {
        ...
        for (ModuleState state : moduleStates.values()) {
            ...
            if (context.inside(event.x(), event.y(), state.toggleX, state.toggleY, state.toggleWidth, state.toggleHeight)) {
                return true;
            }
    ...
    private void renderModuleBindIndicator(Module module, ModuleState state, float nameX, float nameY, float nameSize) {
        ...
        float dotX = nameX + nameWidth + 3.0F + 6.7F + offsetX;
    ...
    private static final class ModuleState {
        ...
        private final Decelerate settingsExpandAnimation = MathUtils.createAnimation(220, Direction.BACKWARDS);
        private boolean settingsExpanded;
        private List<SettingCardComponent<?>> currentSettings;
        ...
        private float animatedCardHeight(SettingCardContext ctx) {
            settingsExpandAnimation.setDirection(settingsExpanded ? Direction.FORWARDS : Direction.BACKWARDS);
            float progress = Math.max(0.0F, Math.min(1.0F, settingsExpandAnimation.getOutput().floatValue()));
            if (currentSettings == null || currentSettings.isEmpty() || progress <= 0.01F) {
                return 24.0F;
            }
            float settingsHeight = 0.0F;
            for (SettingCardComponent<?> component : currentSettings) {
                settingsHeight += component.animatedHeight(ctx);
            }
            float fullHeight = Math.max(34.0F, 24.0F + settingsHeight + 4.0F);
            return 24.0F + (fullHeight - 24.0F) * progress;
        }
    }
