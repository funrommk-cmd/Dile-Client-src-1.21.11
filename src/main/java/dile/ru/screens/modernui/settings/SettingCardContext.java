...
final class SettingCardContext {
    ...
    static Color themeColor() {
        ...
        int rgb = mod != null ? mod.getThemeColor() : ClickGuiTheme.getCycleColor("Purple", 1.0f);
        return new Color((rgb >> 16) & 0xFF, (rgb >> 8) & 0xFF, rgb & 0xFF, 255);
    }

    static Color themeLighter() {
        Color c = themeColor();
        return new Color(Math.min(255, c.getRed() + 50), Math.min(255, c.getGreen() + 20), Math.min(255, c.getBlue() + 10), 255);
    }

    static Color themeDarker() {
        Color c = themeColor();
        return new Color(Math.max(0, c.getRed() - 20), Math.max(0, c.getGreen() - 10), Math.max(0, c.getBlue() - 40), 255);
    }
}
