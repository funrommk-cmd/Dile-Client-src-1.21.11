package dile.ru.utils.render.hands;
...
public final class HandsFlameRenderer {
    private static final Identifier TRAIL_PIPELINE_ID = Identifier.fromNamespaceAndPath("dile", "pipeline/effects/hands_flame_trail");
    private static final Identifier COMPOSITE_PIPELINE_ID = Identifier.fromNamespaceAndPath("dile", "pipeline/effects/hands_flame_composite");
    private static final Identifier FULLSCREEN_VERTEX_SHADER = Identifier.fromNamespaceAndPath("dile", "effects/hands_flame/fullscreen");
    private static final Identifier TRAIL_FRAGMENT_SHADER = Identifier.fromNamespaceAndPath("dile", "effects/hands_flame/trail");
    private static final Identifier COMPOSITE_FRAGMENT_SHADER = Identifier.fromNamespaceAndPath("dile", "effects/hands_flame/composite");
    ...
    private static float flameWidth = 1.0f;
    private static float flameFire = 1.0f;
    private static float flameTurbulence = 1.0f;
    ...
    public static void configure(float strength, float riseSpeed, float wobble, float length, float brightness, int colorMode, int color, boolean itemsOnly, float width, float fire, float turbulence) {
        ...
        flameWidth = clamp(width, 0.1f, 3.0f);
        flameFire = clamp(fire, 0.0f, 3.0f);
        flameTurbulence = clamp(turbulence, 0.0f, 3.0f);
    ...
    private static void ensureTextures(int width, int height) {
        ...
        beforeTexture = createTexture("dile:hands_flame_before", width, height, GpuTexture.USAGE_COPY_DST | GpuTexture.USAGE_TEXTURE_BINDING);
        sceneTexture = createTexture("dile:hands_flame_scene", width, height, GpuTexture.USAGE_COPY_DST | GpuTexture.USAGE_TEXTURE_BINDING);
        trailTextureA = createTexture("dile:hands_flame_trail_a", trailWidth, trailHeight, GpuTexture.USAGE_COPY_DST | GpuTexture.USAGE_TEXTURE_BINDING | GpuTexture.USAGE_RENDER_ATTACHMENT);
        trailTextureB = createTexture("dile:hands_flame_trail_b", trailWidth, trailHeight, GpuTexture.USAGE_COPY_DST | GpuTexture.USAGE_TEXTURE_BINDING | GpuTexture.USAGE_RENDER_ATTACHMENT);
    ...
    private static void writeUniforms(int width, int height) {
        ...
        dataBuffer.putFloat(flameWidth);
        dataBuffer.putFloat(flameFire);
        dataBuffer.putFloat(flameTurbulence);
