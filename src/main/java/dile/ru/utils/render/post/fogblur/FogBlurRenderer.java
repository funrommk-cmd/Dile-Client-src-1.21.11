package dile.ru.utils.render.post.fogblur;
...
public final class FogBlurRenderer {
    ...
    private static final Identifier KAWASE_DOWN_PIPELINE_ID = Identifier.fromNamespaceAndPath("dile", "pipeline/post/fogblur/kawase_down");
    private static final Identifier KAWASE_UP_PIPELINE_ID = Identifier.fromNamespaceAndPath("dile", "pipeline/post/fogblur/kawase_up");
    private static final Identifier COMPOSITE_PIPELINE_ID = Identifier.fromNamespaceAndPath("dile", "pipeline/post/fogblur/composite");
    private static final Identifier KAWASE_VERTEX = Identifier.fromNamespaceAndPath("dile", "post/fogblur/kawase");
    private static final Identifier KAWASE_DOWN_SHADER = Identifier.fromNamespaceAndPath("dile", "ui/kawase/down");
    private static final Identifier KAWASE_UP_SHADER = Identifier.fromNamespaceAndPath("dile", "ui/kawase/up");
    private static final Identifier COMPOSITE_SHADER = Identifier.fromNamespaceAndPath("dile", "post/fogblur/composite");
            ...
            if (compositePipeline == null) {
                compositePipeline = RenderPipelines.register(
                        RenderPipeline.builder()
                                ...
                                .withVertexShader(Identifier.fromNamespaceAndPath("dile", "post/fogblur/fogblur"))
    ...
    private static boolean ensureTargets(int width, int height, float renderScale, int passes) {
        ...
        for (int i = 0; i < passes; i++) {
            downTargets[i] = ensureTarget(downTargets[i], "dile_fog_blur_down_" + i, currentWidth, currentHeight);
            if (i < passes - 1) {
                upTargets[i] = ensureTarget(upTargets[i], "dile_fog_blur_up_" + i, currentWidth, currentHeight);
            }
    ...
    private static void renderKawasePass(RenderPipeline pipeline, GpuTextureView source, int sourceWidth, int sourceHeight, TextureTarget target, float offsetScale, float sourceX, float sourceY, float sourceW, float sourceH, GpuSampler sampler) {
        ...
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            try (RenderPass renderPass = encoder.createRenderPass(() -> "dile:fog_blur_kawase", target.getColorTextureView(), OptionalInt.of(ColorUtil.TRANSPARENT))) {
    ...
    private static void composite(GpuTextureView target, GpuTextureView scene, GpuTextureView blur, GpuTextureView depth, float opacity, float farPlane, float minThreshold, float maxThreshold) {
        ...
        try (RenderPass renderPass = encoder.createRenderPass(() -> "dile:fog_blur_composite", target, OptionalInt.empty())) {
