...
public class TargetFinder {
    ...
    private SortMode resolveSortMode() {
        ...
        return switch (aura.getTargetPriority().getValue()) {
            case "Здоровье" -> SortMode.HEALTH;
            case "Броня" -> SortMode.ARMOR;
            case "Угол обзора" -> SortMode.FOV;
            case "Совмещённый" -> SortMode.COMBINED;
    ...
    public static class EntityFilter {
        ...
        public boolean isValid(LivingEntity entity) {
            ...
            if (entity.isInvisible() && !targetSettings.contains("Invisible") && !targetSettings.contains("Невидимые")) {
                return false;
            }
            if (entity instanceof Player player) {
                AuraModule aura = AuraModule.getInstance();
                boolean antiBotEnabled = aura != null && aura.isEnabled() && aura.getAntiBot().getValue();
                if (antiBotEnabled && AntiBot.shouldIgnore(player)) {
                    return false;
                }
                if (antiBotEnabled && player.getHealth() <= 0.0F) {
                    return false;
                }
                return FriendUtils.isFriend(player) ? (targetSettings.contains("Friends") || targetSettings.contains("Друзья")) : (targetSettings.contains("Players") || targetSettings.contains("Игроки"));
            }
            if (entity instanceof Animal) {
                return targetSettings.contains("Animals") || targetSettings.contains("Животные");
            }
            if (entity instanceof Mob) {
                return targetSettings.contains("Mobs") || targetSettings.contains("Мобы");
            }
            if (entity instanceof ArmorStand) {
                return targetSettings.contains("Armor Stands") || targetSettings.contains("Стойки");
            }
            return false;
        }
    }
