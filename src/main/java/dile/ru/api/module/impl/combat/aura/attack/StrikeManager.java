package dile.ru.api.module.impl.combat.aura.attack;
...
import dile.ru.api.module.impl.combat.aura.target.RaycastAngle;
...
import dile.ru.mixin.accessor.LivingEntityAccessor;
import dile.ru.utils.inventory.lookup.InventoryUtils;
import dile.ru.utils.inventory.swap.SwapExecutor;
import dile.ru.utils.inventory.interaction.PlayerInteractionHelper;
import dile.ru.utils.player.PlayerSimulation;
...
public class StrikeManager implements IMinecraft {
    ...
    public void handleAttack(StrikerConstructor.AttackPerpetratorConfigurable config) {
        ...
        if (raytrace && canAttackNow && !effectiveSprint) {
            ...
            int prevSlot = -1;
            if (config.isShouldBreakShield() && targetIsBlocking(config)) {
                prevSlot = equipAxeForShieldBreak();
            }
            attackEntity(config);
            if (prevSlot >= 0) {
                restoreSlotAfterShieldBreak(prevSlot);
            }
        }
    }
    ...
    private boolean targetIsBlocking(StrikerConstructor.AttackPerpetratorConfigurable config) {
        LivingEntity target = config.getTarget();
        return target != null && target.isBlocking();
    }

    private int equipAxeForShieldBreak() {
        ...
        Slot invSlot = findAxeInInventory();
        if (invSlot != null) {
            int fromSlot = invSlot.index;
            int hotbarTarget = findEmptyHotbarSlot();
            if (hotbarTarget < 0) {
                hotbarTarget = currentSlot;
            }
            InventoryUtils.swapHotbar(fromSlot, hotbarTarget);
            InventoryUtils.selectSlot(hotbarTarget);
            return currentSlot;
        }
        return -1;
    }
    ...
    private void restoreSlotAfterShieldBreak(int slot) {
    ...
    private boolean isSmartCritJumpIntent() {
        ...
        if (accessor.dile$isJumping()) {
            return true;
        }
        if (accessor.dile$getJumpingCooldown() > 0 && !mc.player.onGround()) {
            return true;
        }
