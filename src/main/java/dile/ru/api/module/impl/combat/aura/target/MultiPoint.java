package dile.ru.api.module.impl.combat.aura.target;
...
public class MultiPoint implements IMinecraft {
    ...
    public Tuple<Vec3, AABB> computeVector(LivingEntity entity, float maxDistance, Angle initialAngle, Vec3 velocity, boolean ignoreWalls) {
        ...
        if (isFunTimeMode()) {
            ...
            Vec3 bestVector = findTeleportVector(entity, candidatePoints.getA(), candidatePoints.getB(), initialAngle, currentDataset(), velocity);
            offset = Vec3.ZERO;
        ...
        Vec3 bestVector = findTeleportVector(entity, candidatePoints.getA(), candidatePoints.getB(), initialAngle, dataset, velocity);
        Vec3 fallback = entity.getEyePosition();
        Vec3 result = bestVector == null ? fallback : bestVector;
        if (hasDrift(velocity)) {
            updateOffset(velocity, dataset);
            result = result.add(offset);
        }
        return new Tuple<>(result, candidatePoints.getB());
    }
    ...
    private Vec3 findTeleportVector(LivingEntity entity, List<Vec3> candidatePoints, AABB entityBox, Angle initialAngle, PointDataset dataset, Vec3 velocity) {
        ...
        if (entityChanged || lockedDatasetIndex != datasetIndex || lockedCandidateIndex < 0 || lockedCandidateIndex >= candidatePoints.size() || shouldSwitchPoint) {
            ...
            pointHoldTicks = 0;
            nextPointSwitchTicks = randomPointHoldTicks(hasDrift(velocity));
        }
    ...
    private boolean hasDrift(Vec3 velocity) {
        return velocity != null && (velocity.x != 0.0D || velocity.y != 0.0D || velocity.z != 0.0D);
    }
    ...
    private int randomPointHoldTicks(boolean drift) {
        if (drift) {
            int min = POINT_SWITCH_MIN_TICKS / 3;
            int max = POINT_SWITCH_MAX_TICKS / 3;
            return min + random.nextInt(max - min + 1);
        }
        return randomPointHoldTicks();
    }
