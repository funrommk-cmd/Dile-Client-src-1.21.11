...
public final class InventoryMove extends Module {
    ...
    public boolean shouldSuppressSprintInput() {
        ...
        if (isSlothMode() && isInventoryScreenOpen() && !isClickGuiOpen()) {
            return true;
        }
    ...
    private boolean isSlothMode() {
        return "Sloth".equals(mode.getValue());
    }

    private boolean shouldApplySlothWalk() {
        return isSlothMode()
                && isInventoryScreenOpen()
                && !isClickGuiOpen()
                && InventoryFlowManager.isMovementAllowed();
