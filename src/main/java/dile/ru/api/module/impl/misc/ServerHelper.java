...
import dile.ru.utils.inventory.movement.MovementController;
import dile.ru.utils.inventory.script.Script;
import dile.ru.mixin.accessor.ItemCooldownInstanceAccessor;
import dile.ru.mixin.accessor.ItemCooldownsAccessor;
...
public final class ServerHelper extends Module {
    ...
    private int cooldownRemainingTicks(ItemStack stack) {
        ...
        try {
            ...
            Map<Identifier, ?> activeCooldowns = ((ItemCooldownsAccessor) cooldowns).dile$getCooldowns();
            Object cooldown = activeCooldowns.get(group);
            if (cooldown instanceof ItemCooldownInstanceAccessor accessor) {
                return Math.max(0, accessor.dile$getEndTime() - ((ItemCooldownsAccessor) cooldowns).dile$getTickCount());
            }
