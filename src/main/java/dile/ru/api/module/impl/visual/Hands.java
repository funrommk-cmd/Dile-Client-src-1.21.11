...
import dile.ru.utils.render.hands.HandsFlameRenderer;
...
public class Hands extends Module {
    ...
    private final NumberSetting width = register(new NumberSetting("Width", "Hands flame glow width.", 1.0, 0.1, 3.0, 0.05));
    private final NumberSetting fire = register(new NumberSetting("Fire", "Hands fire spread amount.", 1.0, 0.0, 3.0, 0.05));
    private final NumberSetting turbulence = register(new NumberSetting("Turbulence", "Hands fire chaos level.", 1.0, 0.0, 3.0, 0.05));
    ...
    private int resolveColor() {
        ...
        if (colorMode.is("Client")) {
            ...
            return ColorUtil.lerpColor(ClickGuiModule.getInstance().getColor(0.902F), CLIENT_COLOR_SECOND, pulse);
        }
