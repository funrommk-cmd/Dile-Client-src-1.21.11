...
public final class ParticleSystem {
    ...
    public void spawnTotem(Minecraft client, Entity entity) {
        if (entity == null || client == null || client.level == null || client.player == null || !settings.spawnTotem()) {
            return;
        }
        addParticlesToList(client, entity, Math.round(settings.spawnCountOnTotem.getFloat()), Math.round(settings.aliveTimeBaseOnTotem.getFloat()));
    }

    public void spawnWalk(Minecraft client) {
        if (client == null || client.level == null || client.player == null || !settings.spawnWalk()) {
            return;
        }
        addParticlesToList(client, client.player, Math.round(settings.spawnCountOnWalk.getFloat()), Math.round(settings.aliveTimeBaseOnWalk.getFloat()));
    }
