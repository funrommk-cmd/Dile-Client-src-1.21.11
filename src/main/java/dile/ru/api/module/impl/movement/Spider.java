...
public final class Spider extends Module {
    private final ModeSetting mode = register(new ModeSetting("Mode", "Spider mode.", "Blocks", "Blocks", "FunTimeButtons", "Water", "Boat"));
    private final NumberSetting climbSpeed = register(new NumberSetting("Speed", "Climb speed ticks.", 3.0, 1.0, 10.0, 1.0));
    ...
    public void onTick(Minecraft client) {
        ...
        if (mode.is("Water")) {
            handleWater(client);
        } else if (mode.is("Boat")) {
            handleBoat(client);
        } else if (mode.is("Blocks")) {
            handleBlocks(client);
        } else if (mode.is("FunTimeButtons")) {
            FuntimeButtons(motion);
        }
    }

    private void handleWater(Minecraft client) {
        if (!client.player.horizontalCollision) {
            waterPlaced = false;
            return;
        }

        if (!actionTimer.finished(climbSpeed.getValue().longValue() * 50L)) return;

        if (!waterPlaced) {
            int bucketSlot = findItemSlot(client, Items.WATER_BUCKET);
            if (bucketSlot == -1) {
                setEnabled(false);
                return;
            }

            Direction wallDir = findWallDirection(client);
            BlockPos placePos = client.player.blockPosition().above();
            BlockPos wallBlock = placePos.relative(wallDir);

            int prev = client.player.getInventory().getSelectedSlot();
            client.player.getInventory().setSelectedSlot(bucketSlot);
            client.player.connection.send(new ServerboundSetCarriedItemPacket(bucketSlot));

            BlockHitResult hit = new BlockHitResult(
                    Vec3.atCenterOf(wallBlock), wallDir.getOpposite(), wallBlock, false
            );
            ...
            BlockPos waterPos = client.player.blockPosition().above();
            ...
            BlockHitResult hit = new BlockHitResult(
                    Vec3.atCenterOf(waterPos), Direction.UP, waterPos, false
            );
            client.gameMode.useItemOn(client.player, InteractionHand.MAIN_HAND, hit);
            client.player.swing(InteractionHand.MAIN_HAND);
    ...
    private Direction findWallDirection(Minecraft client) {
        BlockPos playerPos = client.player.blockPosition();
        for (Direction dir : Direction.Plane.HORIZONTAL) {
            if (!client.level.getBlockState(playerPos.relative(dir)).isAir()) {
                return dir;
            }
        }
        return client.player.getDirection();
    }

    private void handleBoat(Minecraft client) {
        if (!client.player.horizontalCollision) {
            if (boatPlaced && actionTimer.finished(200)) {
                boatPlaced = false;
            }
            return;
        }
        ...
        if (!boatPlaced) {
            int boatSlot = findItemSlot(client, Items.OAK_BOAT);
            if (boatSlot == -1) {
                setEnabled(false);
                return;
            }
            ...
            for (Entity entity : client.level.getEntities(client.player, client.player.getBoundingBox().inflate(3.0))) {
    ...
    private void handleBlocks(Minecraft client) {
        ...
        if ((!offHand && slot == -1) || blockPos.equals(BlockPos.ZERO)) return;
    ...
    private int findItemSlot(Minecraft client, net.minecraft.world.item.Item item) {
        for (int i = 0; i < 9; i++) {
    ...
    private Direction getWallDirection() {
        ...
        if (!mc.level.getBlockState(playerPos.relative(forward)).isAir()) return forward;
        for (Direction direction : Direction.Plane.HORIZONTAL) {
            if (!mc.level.getBlockState(playerPos.relative(direction)).isAir()) return direction;
        }
        return forward;
    }
    ...
    private BlockPos findPos(Minecraft client) {
        ...
        if (client.level.getBlockState(blockPos).isCollisionShapeFullBlock(client.level, blockPos)) return BlockPos.ZERO;
        for (BlockPos pos : new BlockPos[]{blockPos.west(), blockPos.east(), blockPos.south(), blockPos.north()}) {
            if (client.level.getBlockState(pos).isCollisionShapeFullBlock(client.level, pos)) return pos;
        }
    ...
    private boolean canPlace(Minecraft client, BlockItem blockItem) {
        BlockPos blockPos = getBlockPos(client);
        if (blockPos.getY() >= BlockPos.containing(client.player.getX(), client.player.getY(), client.player.getZ()).getY()) return false;
    ...
    private static Direction nearestDirection(double x, double y, double z) {
        ...
        if (absX >= absY && absX >= absZ) return x >= 0.0D ? Direction.EAST : Direction.WEST;
        if (absY >= absX && absY >= absZ) return y >= 0.0D ? Direction.UP : Direction.DOWN;
