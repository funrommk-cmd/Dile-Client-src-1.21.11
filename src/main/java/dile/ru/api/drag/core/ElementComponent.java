...
public final class ElementComponent {
    ...
    public float trailX(int index) {
        return index >= 0 && index < trailCount ? trailX[index] : x;
    }

    public float trailY(int index) {
        return index >= 0 && index < trailCount ? trailY[index] : y;
    }
    ...
    void moveTo(float mouseX, float mouseY, ElementScreen screen) {
        ...
        for (int i = Math.min(trailCount, TRAIL_LENGTH - 1); i > 0; i--) {
            trailX[i] = trailX[i - 1];
            trailY[i] = trailY[i - 1];
        }
        trailX[0] = x;
