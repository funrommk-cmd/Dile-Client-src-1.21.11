...
public class HotKeys extends HudPanel {
    protected boolean hud2Mode = false;
    ...
    private HotKeysState logics() {
        ...
        size(width, (Math.max(1, targetRows) - 1) * ROW_STEP + 13.0F + 2.9F);
    ...
    private void renderPanel(HotKeysState state) {
        ...
        FontType iconFont = FontType.MAINMENUSCREEN;
        String iconChar = "m";
        float iconWidth = Render2D.textWidth(iconFont, iconChar, iconSize);
        ...
        int glow = ColorUtil.withAlpha(themeCol, glowAlpha);
        int purple = ColorUtil.withAlpha(themeCol, Math.round(iconAlpha * state.alpha));
        ...
        if (hud2Mode && dile.ru.api.module.impl.visual.Hud2.accentEnabled()) {
            float lineW = 14f;
            float lineH = 3.5f;
            Render2D.rect(state.x + state.width - lineW - 5f, headerY - 0.5f, lineW, lineH, 1.5f, ColorUtil.multAlpha(themeCol, state.alpha));
        }
        ...
        if (glowAlpha > 0) {
            HudRenderCompat.glow("dile:textures/particles/ghost-glow.png", glowX - 13f, glowY + 0.5f, glowSize + 25, glowSize, 0.0F, glow);
        }

        Render2D.text(TEXT_FONT, headerText, headerTextX, headerTextY, headerTextSize, headerTextColor);
        Render2D.text(iconFont, iconChar, iconX, iconY, iconSize, purple);
        ...
        ScissorUtil.push(
                ...
                Render2DCoordinateSpace.toGuiInt(state.y + state.height)
        );
        for (RowState row : state.rows) {
            ...
            float textX = bindX + 2.0F;
            float textY = currentY + 1.0F;
            float padLeft = 2.0F;
            float padRight = 4.0F;
            float padY = 2.0F;
            int cTop = ColorUtil.withAlpha(themeCol, Math.round(100.0F * rowAlpha));
            int cBot = ColorUtil.withAlpha(themeCol, Math.round(5.0F * rowAlpha));
            Render2D.outline(textX - padLeft, textY - padY, bindWidth + padLeft + padRight, row.bindSize + padY * 2, 3.0F, 0.15F, cTop, cTop, cBot, cBot);

            Render2D.text(TEXT_FONT, visibleName, nameX, currentY + 1.5F, 6.0F, ColorUtil.multAlpha(TEXT_COLOR, rowAlpha));
            Render2D.text(TEXT_FONT, row.bind, textX, textY, row.bindSize, ColorUtil.rgba(255, 255, 255, Math.round(245.0F * rowAlpha)));
        }
        ScissorUtil.pop();
    }
