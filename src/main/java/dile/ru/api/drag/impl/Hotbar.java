...
public class Hotbar implements HudElement {
    ...
    private void renderHotbar(HotbarState state) {
        ...
        if (hud2Mode && dile.ru.api.module.impl.visual.Hud2.accentEnabled()) {
            int themeCol = dile.ru.api.module.impl.visual.ClickGuiModule.getInstance().getColor();
            float lineW = 14f;
            float lineH = 3.5f;
            Render2D.rect(state.fullX - state.paddingX + state.fullWidth + state.paddingX * 2.0f - lineW - 5f, state.y - state.paddingY - 0.5f, lineW, lineH, 1.5f, themeCol);
        }
        ...
        GuiGraphics graphics = RenderItem.activeGraphics();
        if (graphics != null) {
            Render2D.flush();
