...
public class Potions extends HudPanel {
    ...
    private void renderPotions(PotionsState state) {
        ...
        if (glowAlpha > 0) {
            renderHudGlow("dile:textures/particles/ghost-glow.png", glowX - 13f, glowY + 0.5f, glowSize + 25, glowSize, 0.0F, glow);
        }
    ...
    private void renderTimer(String text, float x, float y, float size, float alpha) {
        ...
        float textStartX = x + 3.0F;
        float textWidth = text.length() * cellWidth;
        float padX = 2.0F;
        float padY = 2.0F;
        int timerThemeCol = ClickGuiModule.getInstance().getColor();
        int cTop = ColorUtil.withAlpha(timerThemeCol, Math.round(100.0F * alpha));
        int cBot = ColorUtil.withAlpha(timerThemeCol, Math.round(5.0F * alpha));
        Render2D.outline(textStartX - padX, y - padY, textWidth + padX * 2, size + padY * 2, 3.0F, 0.15F, cTop, cTop, cBot, cBot);
    ...
    private static boolean hudBoolean(String fieldName, boolean fallback) {
        try {
            Class<?> hudClass = Class.forName("dile.ru.api.module.impl.visual.Hud");
