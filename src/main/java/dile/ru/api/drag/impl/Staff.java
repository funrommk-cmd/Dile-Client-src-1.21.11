...
public class Staff extends HudPanel {
    ...
    private void renderStaff(StaffState state) {
        ...
        if (glowAlpha > 0) {
            HudRenderCompat.glow("dile:textures/particles/ghost-glow.png", glowX - 13.0F, glowY + 0.5F, glowSize + 25.0F, glowSize, 0.0F, glow);
        }
        ...
        for (RowState row : state.rows) {
            ...
            float dotPad = 2.0F;
            int dotCTop = ColorUtil.withAlpha(themeCol, Math.round(100.0F * rowAlpha));
            int dotCBot = ColorUtil.withAlpha(themeCol, Math.round(5.0F * rowAlpha));
            Render2D.outline(statusX - dotPad, statusY - dotPad, statusSize + dotPad * 2, statusSize + dotPad * 2, 2.0F, 0.15F, dotCTop, dotCTop, dotCBot, dotCBot);
