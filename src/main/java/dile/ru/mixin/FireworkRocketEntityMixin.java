...
import dile.ru.api.module.impl.movement.SuperFireWork;
...
public abstract class FireworkRocketEntityMixin extends Entity {
    ...
    private Vec3 dile$getRotationVectorHook(LivingEntity instance, Operation<Vec3> original) {
        if (dile$shouldModifyFireworkBoost() && attachedToEntity == Minecraft.getInstance().player && attachedToEntity.isFallFlying()) {
    ...
    private void dile$setVelocityHook(LivingEntity instance, Vec3 velocity, Operation<Void> original) {
        if (dile$shouldModifyFireworkBoost() && attachedToEntity == Minecraft.getInstance().player && attachedToEntity.isFallFlying()) {
    ...
    @Unique
    private boolean dile$shouldModifyFireworkBoost() {
