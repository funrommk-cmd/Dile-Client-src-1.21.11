...
public abstract class CreativeInventoryScreenMixin {
    ...
    private void dile$customInvseeCreativeBg(GuiGraphics graphics, RenderPipeline pipeline, Identifier texture, int x, int y, float u, float v, int width, int height, int textureWidth, int textureHeight) {
        ...
        for (CreativeModeTab tab : CreativeModeTabs.tabs()) {
            if (tab != selectedTab) {
                renderTabButton(graphics, 0, 0, tab);
            }
        }
    }
}
