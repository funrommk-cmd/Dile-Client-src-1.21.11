...
float rawHandMaskColor(vec2 uv, float itemOnly, out vec3 outColor) {
    ...
    // The depth buffer is cleared to 1.0 right before the hand renders, so
    // depth < 1.0 means the hand (or an entity) is in front of this pixel.
    // World blocks, clouds and weather have depth 1.0 here and must not seed flame.
    float handDepthMask = texture(DepthSampler, uv).r < 0.9999 ? 1.0 : 0.0;

    return mix(colorMask, itemMask, itemOnly) * handDepthMask;
}
...
float colorMaskAt(vec2 uv) {
    ...
    float value = peak * 0.78 + luma * 0.88 + abs(afterColor.a - beforeColor.a);
    float handDepthMask = texture(DepthSampler, uv).r < 0.9999 ? 1.0 : 0.0;
    return smoothstep(0.004, 0.060, value) * handDepthMask;
}

float nearbyColorMask(vec2 uv, vec2 px, float radiusPx) {
    float mask = colorMaskAt(uv);
    for (int i = 0; i < 8; i++) {
        vec2 probeUv = clamp(uv + dirs[i] * radiusPx * 0.72 * px, vec2(0.0), vec2(1.0));
        mask = max(mask, colorMaskAt(probeUv));
    }
    return mask;
}
...
void main() {
    ...
    float widthScale = params2.x;
    float fireScale = params2.y;
    float turbulenceScale = params2.z;
    float radiusPx = mix(22.0, 62.0, lengthCurve) * widthScale * mix(0.5, 2.0, fireScale);
    ...
    if (quickMask < 0.01 && previous.a < 0.0015 && nearbyColorMask(texCoord, px, radiusPx) < 0.5) {
        fragColor = vec4(0.0);
        return;
    }
    ...
    float turbAmp = mix(0.5, 3.0, turbulenceScale);
    float turbFreq = mix(0.5, 3.0, turbulenceScale);
    float lateralWobble = sin(texCoord.y * 28.0 * turbFreq + time * (1.5 + wobble * 2.5)) * wobble * 0.22 * turbAmp;
    vec2 flameUv = vec2(texCoord.x + lateralWobble, texCoord.y - flow);

    float n1 = fbm(vec2(flameUv.x * 4.4 * turbFreq, flameUv.y * (5.2 + flameLength * 2.7) * turbFreq));
    float n2 = fbm(vec2(flameUv.x * 8.2 * turbFreq + 3.7, flameUv.y * 9.5 * turbFreq - time * 0.35 * turbAmp));
